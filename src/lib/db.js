...
export async function deleteReg(name, id) {
  const q = `DELETE FROM registrations
...
export async function deleteEvent(id) {
  const qr = `DELETE FROM registrations
  WHERE event = $1 `;
  const qe = `DELETE FROM events
  WHERE id = $1 `;
  const resultReg = await query(qr, [id]);
  const resultEve = await query(qe, [id]);
  if (resultEve && resultReg) {
    return 1;
  }
  return null;
}
